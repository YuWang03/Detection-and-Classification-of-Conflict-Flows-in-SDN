...
def _normalize_type(s: str) -> str:
    """Normalize conflict_type text to avoid Normal/normal/None issues."""
    s = str(s).strip()
    if s == "" or s.lower() == "nan":
        return "normal"
    return s.lower()
...
def main():
    ...
    ap.add_argument("--input", default="1000_phase1.csv")
    ...
    # -----------------------------
    # df_eval: ONLY what you want to predict & evaluate
    #   predict_conflict == 1 AND true label != normal
    # -----------------------------
    df_eval = df[
    ...
    # -----------------------------
    # Train Phase2 DT on TRUE conflicts (7 types) from FULL df
    # -----------------------------
    df_true_conf = df[df["conflict_type"].isin(allowed_types)].copy()
    if len(df_true_conf) < 10 or df_true_conf["conflict_type"].nunique() < 2:
        print("[ERROR] Not enough true-conflict samples (or not enough classes) to train Phase2 DT.")
        sys.exit(1)

    X_tc = df_true_conf[["priority", "ip_relation", "action"]]
    y_tc = df_true_conf["conflict_type"]
    ...
    # holdout sanity check (true conflicts only)
    y_pred_te = pipe.predict(X_te)
    acc_holdout = accuracy_score(y_te, y_pred_te)

    print("\n[Phase2] DT trained on TRUE conflicts only (7 classes)")
    ...
    rep = classification_report(
        y_true, y_pred,
        labels=allowed_types,      # enforce 7-class macro
        zero_division=0,
        output_dict=True
    )
    ...
    print(classification_report(y_true, y_pred, labels=allowed_types, zero_division=0))
